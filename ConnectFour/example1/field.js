...
class ConnectFourField {
    ...
    dropStone(col) {
        ...
        if (f[1][col] !== STONE.NONE) return null;

        //該当する列で、フィールドの2行目以降にすでに石があったら、その1つ上の行に石を置く。
        for (let i = 2; i < f.length; i++) {
            if (f[i][col] !== STONE.NONE) {
                f[i - 1][col] = color;
                //ゲーム終了条件を満たしたか判定し、返却する。
                return this._isGameOver(i - 1, col, color);
            }
        }
        //for文を抜けてしまったら論理エラーである。その旨をthrowして通知する。
        throw new Error("dropStone failure!");
    }
    ...
    _isGameOver(row, col, color) {
        ...
        //各探索方向について同じ色でいくつ続いているかを確認し、4以上なら勝利
        for (const p of searchPair) {
            const connectionNum = this._searchConnection(row, col, color, p);
            if (connectionNum >= 4) return GAME_STATE.WIN;
        }

        //表示フィールドが全て埋まっているかをチェック
        let isDraw = true; //引き分けかどうか
        const f = this.getField();

        for (let i = 0; i < f[0].length; i++) { //重力四目並べなので、フィールドの最上部だけ見る。
            //フィールド最上部一つでもSTONE.NONE（＝空きマス）があれば、引き分けではないことがわかる。
            if (f[0][i] === STONE.NONE) {
                isDraw = false;
                break;
            }
        ...
        //全部埋まっていたら引き分け。
        if (isDraw) return GAME_STATE.DRAW;
